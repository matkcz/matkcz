#include "avl_tree.h"
    ...
    void tree::makeEmpty(node* t)
    {
        if (t == nullptr)
            return;
        makeEmpty(t->left); //vyprázdní levý podstrom rekursivnì
        makeEmpty(t->right); //vyprázdní pravý podstrom rekursivnì
        delete t;
    }
    ...
    //int tree::getBalance(node* t) //mìla nahradit porovnávání výšek u ostatních funkcí, ale nefungovalo mi to všude, tak jsem se obešel bez toho
    //{
    //    if (t == nullptr)
    //        return 0;
    //    else
    ...
    tree::node* tree::singleRightRotate(node*& t)
    {
        ...
        t->height = max(height(t->left), height(t->right)) + 1; //zmìna nejvyšší výšky vrcholu t na správnou výšku
        u->height = max(height(u->left), t->height) + 1; //zmìna nejvyšší výšky vrcholu u na správnou výšku
        return u;
    }
    ...
    {
        if (t == nullptr) //když to dojde na prázdné místo (na konci vìtve), kde by to bylo v normálním binárním stromì, tak ho to tam umistí
        {
            t = new node;
            t->data = x;
            t->height = 0;
        ...
        else if (x < t->data) //když je zadané èíslo menší než èíslo aktuálního prvku
        {
            t->left = insert(x, t->left, y); //ho to insertne rekursivnì víc do levého podstromu
            if (height(t->left) - height(t->right) == 2) //když se výška podstromù nìjakého vrcholu liší o 2
            {
        ...
        //cout << t->data<< " ";
        t->height = max(height(t->left), height(t->right)) + 1; //nastavení výšky aktuálních prvkù
        return t;
        //postupnì se rekursivnì vrací až na koøen a dìlá kontroly výšky a popø. odpovídající rotace
    }
    ...
    tree::node* tree::findMax(node* t) //nevyužil jsem protože staèí findMin nebo findMax
    {
        if (t == nullptr)
            return nullptr;
    ...
    {
        //pøi hledaní vrcholu podle stringu nemùžeme používat menší/vìtši pøi výberu odpovídající vìtve
        std::stack<node*> s; //proto vytvoøíme stack(mohla by být fronta) a uložíme všechny vrcholy zde
        node* curr = root;
        ...
        while ((curr != nullptr) || (!s.empty()))
        {
            ...
            s.pop(); //zruší vrch
            i--;
            if (curr->letter == y) //když to najde, tak skonèí while
            {
                break;
            }
    ...
    tree::node* tree::remove(int x, node* t)
    {
        ...
        else //pokud má jednoho nebo žádného potomka
        {
            ...
            if (t->left == nullptr) //pokud levý je nullptr
                t = t->right; //nastaví to t na pravý (v pøípadì žádného na nullptr)
        ...
        else
        {
            ...
            if (height(t->left) - height(t->right) == -2) //pokud není vybalancovaný po odstranìní levého vrcholu
            {
                if (height(t->right->right) - height(t->right->left) == 1) //a pokud rozdíl výšky potomkù pravého potomka je 1 tak dojde k
    ...
    tree::node* tree::remove(string y, node* t) //odstraní vrchol se stringem y
    {
        ...
        remove(curr->data, root); //jinak podle dat èísla nalezeného vrcholu vymaže vrchol
        return t;
    }
    ...
    unsigned int tree::depth() //poèíta výšku stromu
    {
        if (empty()) return 0;
    ...
    //int tree::maxCount() //nakonec jsem nepoužil; returne max poèet prvkù vzhledem k hloubce
    //{
    //    int x = 2;
